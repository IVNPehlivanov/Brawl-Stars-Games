...
export default function StreakBadge() {
  ...
  return (
    <div
      className={`fixed top-4 right-4 z-40 flex items-center gap-1.5 bg-gray-900/90 border border-yellow-400/40 rounded-full px-3 py-1.5 text-sm font-bold text-yellow-400 shadow-lg backdrop-blur transition-transform ${flash ? "scale-110" : "scale-100"}`}
    >
      🔥 {record.streak}
    </div>
  );
