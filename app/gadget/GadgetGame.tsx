...
export default function GadgetGame({ dayKey, onSolved }: Props) {
  const entry = getDailySecretFromPool(GADGET_POOL, "gadget", dayKey);
  ...
  useEffect(() => {
    const saved = getPersistedGameState<PersistedState>("gadget", dayKey);
  ...
  const persist = (g: string[], w: boolean) => {
    setPersistedGameState("gadget", dayKey, { guesses: g, won: w, secretKey });
  };
  ...
  return (
    <div className="space-y-4">
      {/* Gadget clue */}
      <div className="p-4 border border-yellow-400/30 rounded-xl bg-yellow-400/5">
        <p className="text-yellow-400 font-semibold text-sm mb-1">Gadget: {entry.gadgetName}</p>
        <p className="text-white/70 text-sm leading-relaxed">{entry.gadgetDescription}</p>
      </div>
        ...
        <div className="relative">
          <form onSubmit={handleFormSubmit} className="flex gap-2">
            <input
              ...
              placeholder="Which brawler has this gadget?"
              autoComplete="off"
      ...
      {won && (
        <div className="mt-4 p-4 border border-green-500/30 rounded-xl bg-green-500/10 text-center animate-fade-up">
          ...
          <NextModeLink currentSlug="gadget" />
        </div>
      )}
